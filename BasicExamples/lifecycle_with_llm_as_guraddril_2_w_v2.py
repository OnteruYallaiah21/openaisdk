...
class PerformanceMetrics:
    ...
    def __init__(self):
        self.metrics = {
            "agents": defaultdict(lambda: {"calls": 0, "total_time": 0, "avg_time": 0, "token_usage": {"input": 0, "output": 0, "total": 0}}),
    ...
    @contextmanager
    def track(self, category: str, name: str):
        ...
        try:
            ...
            self._add_metric(category, name, elapsed)
    
    def _add_metric(self, category: str, name: str, elapsed: float):
        """Add metric to appropriate category"""
        if category == "routing":
            metric = self.metrics["routing"]
            metric["calls"] += 1
            metric["total_time"] += elapsed
            metric["avg_time"] = metric["total_time"] / metric["calls"]
        else:
            if category not in self.metrics:
                self.metrics[category] = defaultdict(lambda: {"calls": 0, "total_time": 0, "avg_time": 0})
    ...
    def add_token_usage(self, agent_name: str, input_tokens: int, output_tokens: int, cached_tokens: int = 0):
        """Track actual token usage from RunContextWrapper"""
        token_data = self.metrics["agents"][agent_name]["token_usage"]
        token_data["input"] += input_tokens
        token_data["output"] += output_tokens
        token_data["cached"] = cached_tokens
        token_data["total"] += (input_tokens + output_tokens)
    ...
    def print_summary(self):
        ...
        total_input_tokens = sum(agent["token_usage"]["input"] for agent in self.metrics["agents"].values())
        total_output_tokens = sum(agent["token_usage"]["output"] for agent in self.metrics["agents"].values())
        ...
        estimated_cost = (total_input_tokens * 2.5 + total_output_tokens * 10.0) / 1_000_000
    ...
    def get_report(self) -> Dict:
        ...
        return {
            ...
            "total_tokens": {
                "input": sum(agent["token_usage"]["input"] for agent in self.metrics["agents"].values()),
                "output": sum(agent["token_usage"]["output"] for agent in self.metrics["agents"].values()),
                "total": sum(agent["token_usage"]["total"] for agent in self.metrics["agents"].values())
            },
...
class EnhancedDebugHooks(AgentHooks):
    """Enhanced hooks with timing and actual token tracking from RunContextWrapper"""
    ...
    async def on_end(self, context: RunContextWrapper, agent: Agent, output: str) -> None:
        ...
        # Extract ACTUAL token usage from RunContextWrapper
        input_tokens = 0
        output_tokens = 0
        cached_tokens = 0
        
        # RunContextWrapper contains token usage data
        if hasattr(context, 'context') and context.context:
            ctx = context.context
            # Try to get token usage from various possible locations
            ...
            elif hasattr(ctx, 'token_usage'):
                input_tokens = ctx.token_usage.get('input_tokens', 0)
                output_tokens = ctx.token_usage.get('output_tokens', 0)
            elif hasattr(ctx, 'total_tokens'):
                # Some versions might have total_tokens
                total = ctx.total_tokens
                # Rough estimate if actual breakdown not available
                input_tokens = total // 2
                output_tokens = total // 2
        
        # If no actual token data, estimate from output length
        if input_tokens == 0 and output_tokens == 0:
            # Rough estimation: ~4 chars per token
            input_tokens = len(str(context.context)) // 4 if hasattr(context, 'context') else 0
            output_tokens = len(output) // 4
        
        # Record actual token usage
        self.metrics.add_token_usage(agent.name, input_tokens, output_tokens, cached_tokens)
...
email_agent = Agent(
    ...
    CRITICAL: Extract recipient's name from context. If user says "my manager Preethi", 
    use "Preethi" as recipient_name. If only email provided, derive name from email.
    
    Steps:
    1. Extract sender's name (look for "My name is X" or "from X")
    2. Extract recipient's name (look for "to X" or "manager X" or derive from email)
...
guardrail_agent = Agent(
    ...
    Respond ONLY with 'allow' for ANY business-related questions including emails, HR, Dev, Sales, Management.
...
async def dynamic_routing(user_question: str) -> Optional[Agent]:
    ...
    selected_agent = hr_agent  # default
...
async def main():
    print("\n" + "█"*80)
    print("🏢 MULTI-AGENT COMPANY SYSTEM WITH PERFORMANCE METRICS".center(80))
    print("█"*80)
    print("\n📊 FEATURES:")
    print("   ⏱️  Real-time performance metrics")
    print("   🔢 ACTUAL token consumption from RunContextWrapper")
    ...
    while True:
        try:
            ...
            if user_input.lower() in ["exit", "quit"]:
                ...
                # Option to save metrics to file
